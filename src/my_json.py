...
from preparation import MixinPreparation
...
class WorkJson(VacanciesSave, MixinPreparation):
    ...
    def del_vacancies(self, del_id: int):
        """Принимает ID вакансии для удаления, удаляет ее и сохраняет измененный список в Json-файл"""
        all_vacancies = self.get_vacancies()
        count = 0
        for vacans in all_vacancies:
            if vacans["id"] == del_id:
                count += 1
                all_vacancies.remove(vacans)
        self.sav_json(all_vacancies)
        if count == 0:
